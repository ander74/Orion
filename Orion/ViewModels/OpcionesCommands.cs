...
namespace Orion.ViewModels {
    ...
    public partial class OpcionesViewModel {
        ...
        #region COMANDO SINCRONIZAR NUBE PULSADO
        ...
        // Ejecución del comando
        private void SincronizarNubePulsado() {
            // Si se activa el botón y no hay conductores, significa que es la primera vez que se hace y emite el mensaje.
            if (App.Global.Configuracion.SincronizarEnDropbox && App.Global.ConductoresVM.ListaConductores.Count == 0) {

                if (mensajes.VerMensaje("¡ATENCIÓN!\n\n" +
                    "Parece que es la primera vez que se inicia el programa.\n\n" +
                    "¿Quieres descargar los datos de la nube para estar actualizado?",
                               "PRIMER INICIO", true) == false) return;
                try {
                    // Hacemos copia de seguridad de los datos de Dropbox.
                    if (!Respaldo.CopiaDatos()) {
                        if (mensajes.VerMensaje("¡ATENCIÓN!\n\nNo se ha podido hacer una copia de seguridad de los datos.\n\n¿Desea continuar?",
                    ...
                    // Copiamos la carpeta Datos a Dropbox.
                    Respaldo.DropboxToDatos();
                    // Avisamos que sería interesante reiniciar Orión
                    mensajes.VerMensaje("Para que todos los datos se muestren correctamente, reinicia Orión.", "ATENCIÓN");
                } catch (Exception ex) {
                    mensajes.VerError("OpcionesCommands.DropboxAPc", ex);
                }
